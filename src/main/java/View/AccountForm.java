...
public class AccountForm extends javax.swing.JInternalFrame {
    ...
    private ArrayList<Account> accounts = DaoAccount.getInstance().selectAll();
    ...
    /**
     * Đổ danh sách tài khoản vào bảng.
     * Hiển thị trạng thái dạng text (status=1 → "Hoạt động", status=0 → "Đã khóa").
     *
     * @param acc danh sách tài khoản cần hiển thị
     */
    ...
    public Account getAccountSelect() {
        ...
        Account acc = DaoAccount.getInstance().selectById(tblAccount.getValueAt(i_row, 1).toString());
        return acc;
    }
    ...
    private void initComponents() {
        ...
        tblAccount.addComponentListener(new java.awt.event.ComponentAdapter() {
            public void componentShown(java.awt.event.ComponentEvent evt) {
                tblAccountComponentShown(evt);
            }
        });
        ...
        getContentPane().add(jPanel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(-10, 0, 1180, 750));
    ...
    private void btnDeleteAccountActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteAccountActionPerformed
        ...
        if (tblAccount.getSelectedRow() == -1) {
            ...
            if (select.getRole().equals("Admin")) {
                ...
                String username = select.getUser();
                boolean hasPhieuNhap = !DaoPhieuNhap.getInstance().selectallbyaccount(username).isEmpty();
                boolean hasPhieuXuat = !DaoPhieuXuat.getInstance().selectallbyaccount(username).isEmpty();
                if (hasPhieuNhap || hasPhieuXuat) {
                    JOptionPane.showMessageDialog(this,
                        "Tài khoản này đã tạo phiếu, không thể xóa.\nVui lòng dùng chức năng 'Sửa' để thay đổi trạng thái hoạt động.",
                ...
                if (checkVl == JOptionPane.YES_OPTION) {
                    try {
                        DaoAccount.getInstance().delete(select);
                        JOptionPane.showMessageDialog(this, "Xoá thành công tài khoản!");
                        // Refresh cache và bảng sau khi xóa
                        accounts = DaoAccount.getInstance().selectAll();
                        loadDataToTable(accounts);
                    } catch (Exception e) {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                for (int row = 1; row < excelSheet.getLastRowNum(); row++) {
                    ...
                    Account acc = DaoAccount.getInstance().selectById(user);
    ...
    private void txtSearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtSearchKeyReleased
        // Tìm kiếm trong cache accounts (không gọi DB lại mỗi lần gõ phím)
        String luachon = (String) cbxLuachon.getSelectedItem();
        String searchContent = txtSearch.getText();

        // Ánh xạ tên ComboBox sang field cho SearchAccount
        String field;
        switch (luachon) {
            case "Tên tài khoản":  field = "Họ tên";           break;
            case "Tên đăng nhập": field = "Tên đăng nhập";    break;
            case "Email":          field = "Email";             break;
            default:               field = "Tất cả";            break;
        }

        ArrayList<Account> result = SearchAccount.search(accounts, searchContent, field);
        ...
        if (tblAccount.getSelectedRow() == -1) {
            ...
            if (check == JOptionPane.YES_OPTION) {
                ...
                if (textct.equals("")) {
                    ...
                    Account accReset = DaoAccount.getInstance().selectById(userName);
                    // Hash mật khẩu mới bằng BCrypt trước khi lưu vào DB
                    accReset.setPassword(BCrypt.hashPassword(textct));
                    try {
                        DaoAccount.getInstance().update(accReset);
                        JOptionPane.showMessageDialog(this, "Thay đổi mật khẩu thành công!");
                    } catch (Exception e) {
                        JOptionPane.showMessageDialog(this, "Thay đổi không thành công: " + e.getMessage());
                    }
                    // Refresh cache và bảng sau khi cập nhật
                    accounts = DaoAccount.getInstance().selectAll();
                    loadDataToTable(accounts);
                }
            }
    ...
    private void tblAccountComponentShown(java.awt.event.ComponentEvent evt) {//GEN-FIRST:event_tblAccountComponentShown
        // TODO add your handling code here:
    }//GEN-LAST:event_tblAccountComponentShown
