...
public class TonKhoForm extends javax.swing.JInternalFrame {
    ...
    private javax.swing.JButton btnChiTietLo;
    ...
    public TonKhoForm() {
        ...
        changeTextFind();
        initCustomButtons();
        // Double-click dòng sản phẩm → mở dialog chi tiết lô
        tblSanPham.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent e) {
                if (e.getClickCount() == 2 && tblSanPham.getSelectedRow() != -1) {
                    openChiTietLoHang();
                }
            }
        });
    ...
    /**
     * Tải dữ liệu tồn kho từ DB — chỉ hiển thị sản phẩm đang hoạt động (trangThai=1).
     * TonKho và ProductForm dùng chung cùng bảng maytinh, khác nhau ở chỗ TonKho
     * là chế độ xem-only (các nút Thêm/Sửa/Xóa bị disable).
     */
    public void loadDataToTable() {
        try {
            ...
            for (MayTinh i : armt) {
                if (i.getTrangThai() == 1) {
                    // instanceof thay vì gọi thêm DB
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton7ActionPerformed
        // Làm mới: xóa bộ lọc và tải lại dữ liệu từ DB
    ...
    /**
     * Xóa mềm sản phẩm đang chọn (đặt trangThai=0).
     * TonKho không nên thực sự xóa — chỉ ẩn khỏi danh sách hiển thị.
     */
    public void xoaMayTinhSelect() {
        int confirm = JOptionPane.showConfirmDialog(this, "Bạn có muốn xoá sản phẩm này?",
                "Xoá sản phẩm", JOptionPane.YES_NO_OPTION);
        if (confirm == JOptionPane.YES_OPTION) {
            MayTinh remove = getMayTinhSelect();
            remove.setTrangThai(0); // Xóa mềm: không xóa hẳn, chỉ đánh dấu không dùng
            Daomaytinh.getInstance().update(remove);
            loadDataToTable();
        }
    }

    /**
     * Lấy sản phẩm tương ứng dòng đang chọn, dùng maMay ở cột 0 để query DB.
     *
     * @return MayTinh (Laptop hoặc PC) của dòng đang chọn
     */
    ...
    public void loadDataToTableSearch(ArrayList<MayTinh> result) {
        try {
            ...
            for (MayTinh i : result) {
                // TonKho cũng chỉ hiển thị sản phẩm đang hoạt động
                if (i.getTrangThai() == 1) {
                    String loaimay = (i instanceof Laptop) ? "Laptop" : "PC/Case";
                    tblModel.addRow(new Object[]{
                        i.getMaMay(), i.getTenMay(), i.getSoLuong(),
    ...
    /**
     * Lọc danh sách sản phẩm tồn kho theo từ khóa và loại tìm kiếm.
     *
    ...
    public ArrayList<MayTinh> searchFn(String luaChon, String content) {
        // Chỉ tìm trong sản phẩm đang hoạt động
        ArrayList<MayTinh> allActive = new ArrayList<>();
        for (MayTinh m : Daomaytinh.getInstance().selectAll()) {
            if (m.getTrangThai() == 1) allActive.add(m);
        }
        ...
        return SearchProduct.search(allActive, content, field);
    }
    ...
    private void initCustomButtons() {
        ...
        btnChiTietLo = new javax.swing.JButton();
        btnChiTietLo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icon/icons8_eye_40px.png")));
        btnChiTietLo.setText("Chi tiết lô");
        btnChiTietLo.setToolTipText("Xem danh sách lô hàng của sản phẩm");
        btnChiTietLo.setCursor(new java.awt.Cursor(java.awt.Cursor.HAND_CURSOR));
        btnChiTietLo.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        btnChiTietLo.setVerticalTextPosition(javax.swing.SwingConstants.BOTTOM);
        btnChiTietLo.setEnabled(false);
        btnChiTietLo.addActionListener(e -> openChiTietLoHang());
        jToolBar1.add(btnChiTietLo);

        // Bật/tắt nút theo lựa chọn dòng trên bảng
        tblSanPham.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                btnChiTietLo.setEnabled(tblSanPham.getSelectedRow() != -1);
            }
        });
    ...
    private void openChiTietLoHang() {
        if (tblSanPham.getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn sản phẩm !");
            return;
        }
        MayTinh mt = getMayTinhSelect();
        JFrame owner = (JFrame) javax.swing.SwingUtilities.getWindowAncestor(this);
        ChiTietLoHangDialog dialog = new ChiTietLoHangDialog(owner, mt.getMaMay(), mt.getTenMay());
