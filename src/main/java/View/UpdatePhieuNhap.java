...
public class UpdatePhieuNhap extends javax.swing.JDialog {
    ...
    public UpdatePhieuNhap(javax.swing.JInternalFrame parent, javax.swing.JFrame owner, boolean modal) throws UnsupportedLookAndFeelException {
        ...
        // Lay thong tin 
        allProduct = Daomaytinh.getInstance().selectAllExist();
        this.parent = (PhieuNhapForm) parent;
        this.phieunhap = this.parent.getPhieuNhapSelect();
        CTPhieu = DaoChiTietPhieuNhap.getInstance().SelectBymaphieu(phieunhap.getMaPhieu());
        CTPhieuOld = DaoChiTietPhieuNhap.getInstance().SelectBymaphieu(phieunhap.getMaPhieu());
        // Hien thi thong tin
        initTable();
    ...
    private void displayInfo() {
        ...
        txtNguoiTao.setText(DaoAccount.getInstance().selectById(phieunhap.getNguoiTao()).getUser());
    ...
    private void btnNhapHangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNhapHangActionPerformed
        ...
        if (CTPhieu.isEmpty()) {
            ...
            for (var ct : CTPhieuOld) {
                Daomaytinh.getInstance().updateSoLuong(ct.getMaMay(), Daomaytinh.getInstance().selectById(ct.getMaMay()).getSoLuong() - ct.getSoLuong());
            }
            for (var ct : CTPhieu) {
                Daomaytinh.getInstance().updateSoLuong(ct.getMaMay(), Daomaytinh.getInstance().selectById(ct.getMaMay()).getSoLuong() + ct.getSoLuong());
            }
            ...
            try {
                DaoPhieuNhap.getInstance().update(pn);
                DaoChiTietPhieuNhap.getInstance().delete(CTPhieuOld.get(CTPhieuOld.size() - 1));
                for (var i : CTPhieu) {
    ...
    private void addProductActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addProductActionPerformed
        ...
        if (i_row == -1) {
            ...
            if (mtl != null) {
                ...
                MayTinh mt = Daomaytinh.getInstance().selectById((String) tblSanPham.getValueAt(i_row, 0));
