...
public class UpdateProduct extends javax.swing.JDialog {
    ...
    public UpdateProduct(javax.swing.JInternalFrame parent, javax.swing.JFrame owner, boolean modal) {
        ...
        if (this.owner.checklap()) {
            ...
            trangThaiProduct = a.getTrangThai();
    ...
    private void btnAddProductActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddProductActionPerformed
        String maMay = txtMaSanPham.getText().trim();
        String tenMay = txtTenSanPham.getText().trim();
        ...
        if (cbxloaisp.getSelectedItem().equals("Laptop")) {
            double kichthuocman;
            try {
                kichthuocman = Double.parseDouble(txtKichThuocMan.getText().trim());
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập kích thước màn ở dạng số !");
                return;
            }
            if (kichthuocman <= 0) {
                JOptionPane.showMessageDialog(this, "Kích thước màn phải lớn hơn 0 !");
                return;
            }
            String dungluongpin = txtDungLuongPin.getText().trim();
            try {
                double dungluongpinVal = Double.parseDouble(dungluongpin);
                if (dungluongpinVal <= 0) {
                    JOptionPane.showMessageDialog(this, "Dung lượng pin phải lớn hơn 0 !");
                    return;
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập dung lượng pin ở dạng số !");
                return;
            }
            if (maMay.isEmpty() || tenMay.isEmpty() || cpu.isEmpty() || ram.isEmpty() || rom.isEmpty() || gpu.isEmpty() || xuatxu.isEmpty() || dungluongpin.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập đầy đủ thông tin !");
            } else {
                // Cập nhật thông tin Laptop trong DB
                Laptop lp = new Laptop(kichthuocman, dungluongpin, maMay, tenMay, soluong, dongia, cpu, ram, xuatxu, gpu, rom, trangThaiProduct);
                try {
                    Daomaytinh.getInstance().update(lp);
                    this.dispose();
        ...
        if (cbxloaisp.getSelectedItem().equals("PC - Lắp ráp")) {
            String mainboard = txtMainBoard.getText().trim();
            int congsuatnguon;
            try {
                congsuatnguon = Integer.parseInt(txtCongsuatNguon.getText().trim());
            } catch (Exception e) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập công suất nguồn ở dạng số !");
                return;
            }
            if (congsuatnguon <= 0) {
                JOptionPane.showMessageDialog(this, "Công suất nguồn phải lớn hơn 0 !");
                return;
            }
            if (maMay.isEmpty() || tenMay.isEmpty() || cpu.isEmpty() || ram.isEmpty() || rom.isEmpty() || gpu.isEmpty() || xuatxu.isEmpty() || mainboard.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Vui lòng nhập đầy đủ thông tin !");
            } else {
                // Cập nhật thông tin PC trong DB
                PC pc = new PC(mainboard, congsuatnguon, maMay, tenMay, soluong, dongia, cpu, ram, xuatxu, gpu, rom, trangThaiProduct);
                try {
                    Daomaytinh.getInstance().update(pc);
                    this.dispose();
                    JOptionPane.showMessageDialog(this, "Sửa sản phẩm thành công !");
                    owner.loadDataToTable();
                } catch (Exception e) {
