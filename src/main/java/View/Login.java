...
public class Login extends javax.swing.JFrame {
     ...
     */
    // Màu nền nút đăng nhập: mặc định, khi hover, khi nhấn
    ...
    /**
     * Kiểm tra thông tin đăng nhập và mở màn hình tương ứng với role của tài khoản.
     * Được gọi khi nhấn nút đăng nhập hoặc nhấn Enter ở ô username/password.
     */
    public void checkLogin() {
        String usercheck = loginUser.getText().trim();
        // Lấy mật khẩu từ JPasswordField (dùng getText() cho đơn giản trong nội bộ)
        ...
        try {
            // Truy vấn tài khoản từ database theo username
            Account acc = DaoAccount.getInstance().selectById(usercheck);
            ...
            // So sánh mật khẩu nhập vào với hash BCrypt đã lưu trong DB
            if (!BCrypt.checkPassword(passwordcheck, acc.getPassword())) {
                JOptionPane.showMessageDialog(this, "Sai mật khẩu !", "Cảnh báo !", JOptionPane.WARNING_MESSAGE);
                return;
            }

            // Kiểm tra tài khoản có bị khóa không (status = 0 là bị khóa)
            if (acc.getStatus() != 1) {
                JOptionPane.showMessageDialog(this, "Tài khoản của bạn đã bị khóa !", "Cảnh báo !", JOptionPane.WARNING_MESSAGE);
                return;
            }

            // Đăng nhập thành công — đóng cửa sổ login, mở giao diện theo role
            this.dispose();
            String role = acc.getRole();

            if (role.equals("Admin")) {
                // Admin có toàn quyền — mở màn hình quản trị đầy đủ
                Admin ad = new Admin(acc);
                ad.setVisible(true);
                ad.setName(acc.getFullname());

            } else if (role.equals("Quản lý kho")) {
                // Quản lý kho: xem tổng quan, báo cáo
                ...
                // Nhân viên nhập: chỉ tạo phiếu nhập hàng
                NhapKho nk = new NhapKho(acc);
                nk.setVisible(true);
                nk.setName(acc.getFullname());

            } else if (role.equals("Nhân viên xuất")) {
                // Nhân viên xuất: chỉ tạo phiếu xuất hàng
                XuatKho xk = new XuatKho(acc);
                xk.setVisible(true);
                xk.setName(acc.getFullname());

            } else {
                // Role không xác định — thông báo lỗi
                JOptionPane.showMessageDialog(this, "Role tài khoản không hợp lệ: " + role, "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
